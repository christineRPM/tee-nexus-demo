...
async function deployLogoHuntGame(cfg: any) {
  const signer = getSigner(cfg.rpc);
  const mailbox = chainAddresses[cfg.name as keyof typeof chainAddresses].mailbox;
  const LogoHuntGameFactory = new LogoHuntGame__factory(signer);
  const logoHuntGame = await LogoHuntGameFactory.deploy(mailbox, ethers.constants.AddressZero, {
    gasLimit: 5000000
  });
  await logoHuntGame.deployTransaction.wait();
  const address = logoHuntGame.address;
  console.log(`${cfg.name} LogoHuntGame deployed at:`, address);
...
async function main() {
  ...
  for (const [_, cfg] of Object.entries(CHAIN_CONFIG)) {
    const mailbox = chainAddresses[cfg.name as keyof typeof chainAddresses].mailbox;
    const deployed = await deployLogoHuntGame(cfg);
  ...
  writeFileSync(
    `${deploymentsDir}/logohuntgame.json`,
    JSON.stringify(deployments, null, 2)
  );
  console.log(`Deployment details written to ${deploymentsDir}/logohuntgame.json`);
  ...
  for (const [_, cfg] of Object.entries(CHAIN_CONFIG)) {
    ...
    const tx1 = await logoHuntGame.enrollRemoteRouter(remote.chainId, abiEncodedRouter);
    await tx1.wait();
    ...
    const tx2 = await logoHuntGame.setInterchainSecurityModule(cfg.interchainSecurityModule);
    await tx2.wait();
